...
*/
    ...
    async getMyTokens(@Request() req: any) {
        ...
        return this.tokenService.findAll().then(tokens => 
            tokens.filter(token => token.user.id === userId)
        );
    }
    ...
    async deleteMyToken(@Param('id') id: string, @Request() req: any) {
        const userId = req.user.userId;
        if (!id || !userId) {
            throw new HttpException('Token ID and User ID are required', HttpStatus.BAD_REQUEST);
        }
        return this.tokenService.delete(id).then(() => {
            this.tokenService.findAll().then(tokens => 
                tokens.filter(token => token.user.id === userId)
            );
        });
    }
