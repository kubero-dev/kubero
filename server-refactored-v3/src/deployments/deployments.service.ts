...
export class DeploymentsService {
  ...
  public async listBuildjobs(pipelineName: string, phaseName: string, appName: string): Promise<any> {
      ...
      const appresult = await this.appsService.getApp(pipelineName, phaseName, appName)
      ...
      let retJobs = [] as IKuberoBuildjob[]
  ...
  public async triggerBuildjob(
      ...
      if ( process.env.KUBERO_READONLY == 'true'){
          this.logger.log('KUBERO_READONLY is set to true, not triggering build for app: '+app + ' in pipeline: '+pipeline);
          return;
      }
      ...
      try {
          ...
          this.logger.error('kubectl.createBuildJob: Error creating Kubero build job', error)
      }
  ...
  public async deleteBuildjob(pipeline: string, phase: string, app: string, buildName: string, user: IUser): Promise<any> {

      if ( process.env.KUBERO_READONLY == 'true'){
          this.logger.log('KUBERO_READONLY is set to true, not creating app: '+app + ' in pipeline: '+pipeline);
          return;
      }
      ...
      if (contextName) {
        ...
        for (const pod of pods) {
          ...
          if (pod.metadata?.labels?.kuberoapp == appName && pod.metadata.name && pod.metadata?.labels?.["job-name"] == buildName) {
            const ll = await this.LogsService.fetchLogs(namespace, pod.metadata.name, containerName, pipelineName, phaseName, appName)
