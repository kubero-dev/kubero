...
import { MetricsOptions, IMetric, PrometheusQuery, Rule } from './metrics.interface';
...
export class MetricsService {
  ...
  constructor(
      ...
      this.prom.status().then((status) => {
          Logger.log('✅ Feature: Prometheus Metrics initialized::: '+ options.endpoint, 'Feature');
          this.status = true;
      }).catch((error) => {
          Logger.log('❌ Feature: Prometheus not accesible ...', 'Feature');
          this.status = false;
      })
