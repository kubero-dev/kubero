...
export class Oauth2Strategy extends PassportStrategy(Strategy) {
  constructor() {
    ...
    if (!ENV?.OAUTO2_CLIENT_AUTH_URL) {
      logger.error('OAUTO2_CLIENT_AUTH_URL is not defined in the environment variables');
      return;
    }
    if (!ENV?.OAUTO2_CLIENT_TOKEN_URL) {
      logger.error('OAUTO2_CLIENT_TOKEN_URL is not defined in the environment variables');
      return;
    }
    if (!ENV?.OAUTH2_CLIENT_ID) {
      logger.error('OAUTH2_CLIENT_ID is not defined in the environment variables');
      return;
    }
    if (!ENV?.OAUTH2_CLIENT_SECRET) {
      logger.error('OAUTH2_CLIENT_SECRET is not defined in the environment variables');
      return;
    }
    if (!ENV?.OAUTH2_CLIENT_CALLBACKURL) {
      logger.error('OAUTH2_CLIENT_CALLBACKURL is not defined in the environment variables');
      return;
    }
  ...
  async validate(accessToken: string, _refreshToken: string, profile: any) { //TODO: replace 'any' with 'Profile'
